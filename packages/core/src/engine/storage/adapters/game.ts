...
import type { KeysOfType, Payload, PayloadMap } from '../../primitives';
...
export class GameStorageAdapter extends VersionedStorage {
	...
	> {
		...
		const gameRecord = gameInsert.data;
		const playerRecordsWithPublicUserInfo = playerInserts.map(({ data }) => {
	...
	async applyAction(game: Game, action: Action): Promise<string> {
		const { gameRecord, playerRecordsWithPublicUserInfo } =
			GameMapper.toRecord(game);
		...
		await this.dataSource.performAtomic([
			...
			{ type: 'insert', table: TABLES.ACTIONS, data: actionRecord },
		]);
	...
	async endGame(
		game: Game,
		profiles: Record<string, { profile: Profile; winner: boolean }>,
		isDraw: boolean,
	): Promise<void> {
		...
		for (const userId in profiles) {
			const { profile, winner } = profiles[userId];

			const statsDelta: Partial<
				Record<KeysOfType<ProfileRecord, number>, number>
			> = isDraw ? { draws: 1 } : winner ? { wins: 1 } : { losses: 1 };

			operations.push(
				this.prepareDeltaUpdates('profiles', userId, profile.version, {
					totalGames: 1,
					...statsDelta,
				}),
			);
